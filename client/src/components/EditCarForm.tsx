import { CarDetail, Category } from "@/types/interfaces";
...
function EditCarForm() {
  ...
  const setValues = (carDetail: CarDetail) => {
    form.setValue("status", carDetail.status);
    form.setValue("make", carDetail.make);
    form.setValue("model", carDetail.model);
    form.setValue("productionYear", carDetail.productionYear);
    form.setValue("power", carDetail.power);
    form.setValue("fuelType", carDetail.fuelType);
    form.setValue("transmissionType", carDetail.transmissionType);
    form.setValue("driveType", carDetail.driveType);
    form.setValue("priceForDay", carDetail.price.forDay);
    form.setValue("priceForTwoToFourDays", carDetail.price.forTwoToFourDays);
    form.setValue("priceForWeek", carDetail.price.forWeek);
    form.setValue("categoryId", carDetail.category.id);
  };
    ...
    fetch(`http://localhost:8080/api/cars/${id}`, {
      ...
      .then((res) => {
        if (res.ok) {
          toast({
            title: "Car",
            description: "Car update has been processed successfully",
          });
          navigate("/dashboard");
        } else {
          return res.json();
